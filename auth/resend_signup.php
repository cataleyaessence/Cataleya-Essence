<?php
// auth/resend_signup.php – called via auth.php?action=resend
...
// Check cooldown: prevent resending within 60 seconds
$stmt = $pdo->prepare("SELECT created_at FROM otp_codes WHERE user_id = ? AND purpose = 'signup' ORDER BY created_at DESC LIMIT 1");
$stmt->execute([$user_id]);
$lastOtp = $stmt->fetch();
if ($lastOtp) {
...
// Send email
$mailSent = sendOTPEmail($user['email'], $otp, 'signup');
